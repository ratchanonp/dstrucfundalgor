...
public class SparseVector implements Vector {
    ...
    public Vector add(Vector v) throws Exception {
        if (this.length() != v.length()) throw new Exception("Not same size");

        Vector result = new DenseVector(this.length);
        for (int i = 0; i < this.length; i++) result.set(i, this.get(i) + v.get(i));
        return result;
    }
    ...
    public SparseVector add(SparseVector v2) {
        SparseVector v1 = this;
        SparseVector result = new SparseVector(v1.length());
        int i1 = 0, i2 = 0, i3 = 0;
        while (i1 < v1.size && i2 < v2.size) {
            Element e1 = v1.elementData[i1], e2 = v2.elementData[i2];
            if (e1.index < e2.index) {
                result.add(i3++, e1.index, e1.value);
                i1++;
            } else if (e1.index > e2.index) {
                result.add(i3++, e2.index, e2.value);
                i2++;
            } else {
                result.add(i3++, e1.index, e1.value + e2.value);
                i1++;
                i2++;
            }
        }
    ...
    public String toString() {
        String result = "[ ";
        for (int i = 0; i <= elementData[size - 1].index; i++) {
            result += this.get(i) + " ";
        }
        result += "]";
        return result;
